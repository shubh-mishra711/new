...
const CurrencyConverter = () => {
  ...
  const [previousCountryDetails, setPreviousCountryDetails] = useState(null);
  const [previousWeather, setPreviousWeather] = useState(null);

  useEffect(() => {
    if (countryDetails) {
      setPreviousCountryDetails(countryDetails);
    }
  }, [countryDetails]);
  ...
  return (
    ...
    <div className="box">
      ...
      <div className="Currency">
        ...
        {countries.length > 0 && (countryDetails || previousCountryDetails) && (weather || previousWeather) && (
            ...
            <div className="country-details">
              ...
              <div className="country-details-content">
                <div className="flag-container">
                  <img
                    src={(countryDetails || previousCountryDetails).flags.png}
                    alt={(countryDetails || previousCountryDetails).name.common}
                    className="country-flag"
                  />
                </div>
                <div className="country-details-info">
                  <p>Name: {(countryDetails || previousCountryDetails).name.common}</p>
                  <p>Capital: {(countryDetails || previousCountryDetails).capital[0]}</p>
                  <p>Population: {(countryDetails || previousCountryDetails).population}</p>
            ...
            <div className="weather-details">
              ...
              <div className="weather-content">
                <div className="weather-icon-container">
                  <img
                    src={`http://openweathermap.org/img/wn/${(weather || previousWeather).weather[0].icon}.png`}
                    alt="Weather Icon"
                    className="weather-icon"
                  />
                </div>
                <div className="weather-info">
                  <p>Location: {(countryDetails || previousCountryDetails).name.common}</p>
                  <p>Temperature: {(weather || previousWeather).main.temp}°C</p>
                  <p className="weather-w">Weather: {(weather || previousWeather).weather[0].description}</p>
                  <p className="weather-humidity">Humidity: {(weather || previousWeather).main.humidity}%</p>
                  <p className="weather-wind">Wind: {(weather || previousWeather).wind.speed} km/h</p>
                  <p className="weather-pressure">Pressure: {(weather || previousWeather).main.pressure} hPa</p>
